...
def process_line(img_list):
    #print img_list
    target = int(img_list[1][-1]) # cX or test_cX
    #print img_list
    if 'test' in img_list[1]: # image in test directory
        img_path = '../input/imgs/test/' + img_list[2]
    else: # images in train directory
...
def final_training():
    ...
    top_model.add(Dense(2560, init='he_normal'))
...
def fine_tuning():
    ...
    for train_drivers, test_drivers in kf:
        ...
        sample_rate_test = np.random.uniform(0.1, 0.44)
        print 'sample_rate_test:', sample_rate_test

        preds_test = pd.read_csv('submission/ensemble1_5.csv')

        tmp = preds_test.iloc[:,:10].max(1)
        tmp_idx = tmp.iloc[(tmp > 0.80).values].index
        ...
        try:
            ...
            # random split
            np.random.seed(fold_number)
            split_data = np.random.uniform(0.05, 0.10)
